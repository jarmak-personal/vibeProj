...
This skill is the landing gate for vibeProj. Every commit must pass
...
3. Categorize the changed files:
   - **kernel/GPU code** (`fused_kernels.py`, `helmert.py`, `_ds_device_fns.py`,
     `runtime.py`, CUDA source strings): needs GPU code review + performance +
     zero-copy analysis
   - **pipeline/transform** (`pipeline.py`, `transformer.py`): needs
     performance + zero-copy analysis
   - **projection xp** (`projections/*.py`): needs maintainability
   - **CRS/ellipsoid** (`crs.py`, `ellipsoid.py`): needs maintainability
...
| Category | Agent | When |
|----------|-------|------|
| GPU Code Review | `gpu-code-reviewer` | kernel/GPU/NVRTC code changed |
| Zero-Copy | `zero-copy-reviewer` | pipeline/transform/kernel code changed |
| Performance | `performance-reviewer` | kernel/pipeline/transform code changed |
| Maintainability | `maintainability-reviewer` | any non-test source code changed |

Prompt template for each agent:

```
Review the following changes for vibeProj.
...
#### Maintainability: [CONSISTENT / GAPS / BROKEN]
[findings or "N/A"]
...
The marker is single-use — delete it after a successful commit.
